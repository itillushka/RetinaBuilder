"""
Alignment functions adapted for 2D averaged B-scans.

This module provides Y-alignment and rotation alignment specifically
for averaged B-scans, reusing the proven contour-based methods from
...
def perform_x_fine_tuning_2d(bscan_ref, bscan_mov, search_range=600, verbose=True):
    """
    Fine-tune X-alignment on two 2D averaged B-scans using contour-based method.

    This refines the coarse X-alignment from phase correlation by analyzing
    vertical tissue edges in the averaged B-scans.

    Args:
        bscan_ref: Reference B-scan (Y, X)
        bscan_mov: Moving B-scan to align (Y, X)
        search_range: X-shift search range in pixels (default: 600)
        verbose: Print detailed information (default: True)

    Returns:
        dict containing:
            - 'x_fine_tune': Fine-tuning X-shift to apply
            - 'ncc_score': NCC score at best offset
            - 'confidence': Alignment confidence
    """
    if verbose:
        print("\n" + "="*70)
        print("X-AXIS FINE-TUNING (2D Contour-Based)")
        print("="*70)
    ...
    # Preprocess B-scans
    b0_proc = preprocess_oct_for_visualization(bscan_ref)
    b1_proc = preprocess_oct_for_visualization(bscan_mov)

    # Create vertical profiles (sum along Y-axis)
    profile_ref = np.sum(b0_proc, axis=0)  # (X,)
    profile_mov = np.sum(b1_proc, axis=0)  # (X,)
    ...
    # Search for best X-offset using cross-correlation
    ...
    if best_offset >= 0:
        b0_crop = b0_proc[:, best_offset:]
        b1_crop = b1_proc[:, :b0_crop.shape[1]]
    else:
        b1_crop = b1_proc[:, -best_offset:]
        b0_crop = b0_proc[:, :b1_crop.shape[1]]

    ncc_score = calculate_ncc(b0_crop, b1_crop)

    if verbose:
        print(f"  Best X fine-tune offset: {best_offset:+d} px")
        print(f"  Profile correlation: {best_score:.4f}")
        print(f"  Full image NCC: {ncc_score:.4f}")
        print("="*70)

    return {
        'x_fine_tune': int(best_offset),
