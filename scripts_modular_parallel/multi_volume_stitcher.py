...
from helpers.visualization import generate_3d_visualizations, visualize_multi_volume_panorama
...
class MultiVolumeStitcher:
    ...
    def align_volume_pair(self,
        ...
        # Get position for directional constraints
        mov_position = self.graph.nodes[mov_id].position
        ref_position = self.graph.nodes[ref_id].position

        logger.info(f"\n{'='*70}")
        logger.info(f"ALIGNING VOLUME {mov_id} ({mov_position}) → {ref_id} ({ref_position})")
        logger.info(f"{'='*70}")
        ...
        # Apply directional constraints for cross-pattern layout
        from helpers.direction_constraints import constrain_offset_to_direction
        from scipy import ndimage

        offset_x_raw = step1_results['offset_x']
        offset_z_raw = step1_results['offset_z']

        logger.info(f"  Raw XZ offset: dx={offset_x_raw:.1f}, dz={offset_z_raw:.1f}")

        # Constrain to expected direction (10% tolerance for perpendicular movement)
        offset_x_constrained, offset_z_constrained = constrain_offset_to_direction(
            offset_x_raw,
            offset_z_raw,
            mov_position,
            tolerance=0.1
        ...
        logger.info(f"  Constrained XZ offset: dx={offset_x_constrained:.1f}, dz={offset_z_constrained:.1f}")

        # Re-apply constrained shift
        volume_1_xz_aligned = ndimage.shift(
            mov_volume, shift=(0, offset_x_constrained, offset_z_constrained),
            order=1, mode='constant', cval=0
        )

        # Update step1_results with constrained values
        step1_results['volume_1_xz_aligned'] = volume_1_xz_aligned
        step1_results['offset_x'] = offset_x_constrained
        step1_results['offset_z'] = offset_z_constrained
        step1_results['offset_x_raw'] = offset_x_raw
        step1_results['offset_z_raw'] = offset_z_raw
        ...
        # Step 3: Rotation Alignment (axis depends on position)
        logger.info("\nStep 3: Rotation Alignment (Position-Aware)")
        step3_results = perform_z_rotation_alignment(
            ref_volume,
            volume_aligned,
            visualize=False,  # Disable visualization in parallel mode
            position=mov_position  # Pass position for rotation axis selection
        )
    ...
    def process_all_levels(self):
        ...
        for level_idx, vol_ids in enumerate(levels):
            ...
            # Generate intermediate visualization after this level
            logger.info(f"\nGenerating intermediate visualization after Level {level_idx}...")
            self._visualize_intermediate_state(level_idx)

        logger.info("\n✓ All levels processed successfully")

    def _visualize_intermediate_state(self, level_idx):
        """
        Generate intermediate visualization showing volumes aligned so far.

        Args:
            level_idx: Current level index for filename
        """
        # Get canvas size for current volumes
        canvas_size = self.coord_system.get_global_canvas_size()

        # Create temporary merger
        temp_merger = ProgressiveCanvasMerger(
            initial_shape=canvas_size,
            merge_strategy='average',
            track_sources=True
        )

        # Add all aligned volumes so far
        for vol_id in sorted(self.aligned_volumes.keys()):
            ...
            temp_merger.add_volume(volume, offset, vol_id)

        # Finalize
        merged_volume, metadata = temp_merger.finalize()
        source_labels = temp_merger.get_source_labels()

        # Generate visualization
        if source_labels is not None:
            from helpers.visualization import visualize_multi_volume_panorama

            output_path = self.output_dir / f'intermediate_level{level_idx}_panorama.png'

            print(f"\n  Visualizing {len(self.aligned_volumes)} volumes after Level {level_idx}...")
            ...
            # Rename to include level info
            final_path = self.output_dir / f'intermediate_level{level_idx}_panorama.png'
            if (self.output_dir / '3d_panorama_9volumes_multiangle.png').exists():
                import shutil
                shutil.move(
                    self.output_dir / '3d_panorama_9volumes_multiangle.png',
                    final_path
                )
                logger.info(f"  ✓ Saved intermediate visualization: {final_path}")
    ...
    def generate_visualizations(self, canvas: np.ndarray):
        ...
        # Generate 3D color-coded visualization
        source_labels = self.merger.get_source_labels()
        if source_labels is not None:
            visualize_multi_volume_panorama(
            ...
            logger.info("✓ Generated 3D panorama visualization")
        else:
            logger.warning("⚠ Source labels not available, skipping 3D visualization")
