...
class OCTVolumeLoader:
    ...
    def load_volume_from_directory(self, data_dir: str) -> Optional[np.ndarray]:
        """
        Load all BMP files from directory and create 3D volume.
        ...
        try:
            ...
            for i, file_path in enumerate(bmp_files):
                if i % 50 == 0 or i == 0:
                    logger.info(f"Loading scan {i+1}/{num_slices}")

                img = self.processor.load_image(file_path)
                if img is not None:
                    volume[:, :, i] = img
                else:
                    logger.warning(f"Skipped corrupted file: {file_path}")
