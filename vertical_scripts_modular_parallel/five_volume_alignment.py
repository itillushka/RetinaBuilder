...
def align_volume_pair(ref_volume, mov_volume, position, data_dir, prefix, visualize=False):
    ...
    """
    # XZ alignment (VERTICAL: limit X, unlimited Z)
    print(f"\n  [XZ] XZ Alignment...")
    xz_results = perform_xz_alignment(
        ref_volume, mov_volume,
        max_offset_x=15,  # VERTICAL: limit X instead of Z
        method='phase_corr',
        output_dir=data_dir
    )
    ...
    # VERTICAL: pass offset_z instead of offset_x for overlap calculation
    print(f"\n  [Y] Y Alignment (overlap-cropped for calculation)...")
    y_results = perform_y_alignment(
        ref_volume,
        xz_results['volume_1_xz_aligned'],
        position=position,
        offset_z=xz_results['offset_z'],  # VERTICAL: use Z offset
        output_dir=data_dir,
        prefix=prefix
    )
...
Examples:
  ...
  python five_volume_alignment.py --patient EM005 --visual
    ...
    if len(patient_vols) < 9:
        raise ValueError(f"Need at least 9 volumes for patient {args.patient}, found {len(patient_vols)}")

    print(f"  Found {len(patient_vols)} volumes for patient {args.patient}")
    print(f"  Volume 1 (center): {patient_vols[0].name}")      # Folder 5
    print(f"  Volume 2 (right):  {patient_vols[5].name}")      # Folder 6
    print(f"  Volume 3 (far right): {patient_vols[6].name}")   # Folder 7
    print(f"  Volume 4 (left):   {patient_vols[7].name}")      # Folder 8
    print(f"  Volume 5 (far left): {patient_vols[8].name}")    # Folder 9

    volume_1 = loader.load_volume_from_directory(str(patient_vols[0]))  # Folder 5 (center)
    volume_2 = loader.load_volume_from_directory(str(patient_vols[5]))  # Folder 6
    volume_3 = loader.load_volume_from_directory(str(patient_vols[6]))  # Folder 7
    volume_4 = loader.load_volume_from_directory(str(patient_vols[7]))  # Folder 8
    volume_5 = loader.load_volume_from_directory(str(patient_vols[8]))  # Folder 9
    ...
    alignment_data = {
        ...
        "volumes": {
            "v1": {
                "path": str(patient_vols[0]),  # Folder 5 (center)
                "role": "reference",
                "dx": 0,
                "dy": 0,
                "dz": 0,
            ...
            "v2": {
                "path": str(patient_vols[5]),  # Folder 6
            ...
            "v3": {
                "path": str(patient_vols[6]),  # Folder 7
            ...
            "v4": {
                "path": str(patient_vols[7]),  # Folder 8
            ...
            "v5": {
                "path": str(patient_vols[8]),  # Folder 9
