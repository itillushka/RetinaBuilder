...
# VERTICAL PIPELINE: Uses X-sections (Y, Z), so all positions need X-rotation (YZ plane)
# This rotates each X-section (Y, Z slice) which aligns layers across the volume
ROTATION_AXES = {
    'right': (0, 2),       # X-rotation (YZ plane) - VERTICAL: volumes stacked in Z
    'far_right': (0, 2),   # X-rotation (YZ plane)
    'left': (0, 2),        # X-rotation (YZ plane)
    'far_left': (0, 2),    # X-rotation (YZ plane)
    ...
    'center': (0, 2)       # X-rotation for vertical volumes
}
