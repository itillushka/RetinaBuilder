...
def apply_rotation_z_parallel(volume, angle_degrees, axes=(0, 1), n_jobs=-1):
    """
    Parallel rotation using OpenCV.
    ...
    Args:
        ...
        axes: Rotation axes:
              - (0, 1): Z-axis rotation (YX plane) - for horizontal volumes
              - (0, 2): X-axis rotation (YZ plane) - for vertical volumes
        n_jobs: Number of parallel jobs (-1 = all cores)

    Returns:
        Rotated volume (same shape as input)
    """
    def rotate_single_slice(slice_2d, angle):
        """Rotate a single 2D slice using OpenCV."""
        H, W = slice_2d.shape
        center = (W // 2, H // 2)
    ...
    if axes == (0, 1):
        # Z-axis rotation: rotate each B-scan (Y, X) across Z dimension
        # This is for HORIZONTAL volumes
        rotated_slices = Parallel(n_jobs=num_workers, backend='threading', verbose=0)(
            delayed(rotate_single_slice)(volume[:, :, z], angle_degrees)
            for z in range(volume.shape[2])
        )
        return np.stack(rotated_slices, axis=2)

    elif axes == (0, 2):
        # X-axis rotation: rotate each X-section (Y, Z) across X dimension
        # This is for VERTICAL volumes
        rotated_slices = Parallel(n_jobs=num_workers, backend='threading', verbose=0)(
            delayed(rotate_single_slice)(volume[:, x, :], angle_degrees)
            for x in range(volume.shape[1])
        )
        return np.stack(rotated_slices, axis=1)

    else:
        raise NotImplementedError(f"Only axes=(0,1) and axes=(0,2) supported, got {axes}")
